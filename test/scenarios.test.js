...
describe("Required Scenarios", () => {
  test("Scenario 1: 3 x ult_small, 1 x ult_large", () => {
    ...
    assert.strictEqual(cart.total, 94.7);

    const ultSmallItems = cart.items.filter(
      (item) => item.product.code === "ult_small",
    );
    const ultLargeItems = cart.items.filter(
      (item) => item.product.code === "ult_large",
    );
  ...
  test("Scenario 2: 2 x ult_small, 4 x ult_large", () => {
    ...
    assert.strictEqual(cart.total, 209.4);
  ...
  test("Scenario 3: 1 x ult_small, 2 x ult_medium", () => {
    ...
    assert.strictEqual(cart.total, 84.7);
    assert.strictEqual(cart.items.length, 3);

    const ultSmallItems = cart.items.filter(
      (item) => item.product.code === "ult_small",
    );
    const ultMediumItems = cart.items.filter(
      (item) => item.product.code === "ult_medium",
    );
    const dataPackItems = cart.items.filter(
      (item) => item.product.code === "1gb",
    );
